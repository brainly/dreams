...
export default function convertAngleToFromAndTo(angle, width, height) {
  ...
  function calculateFromAndTo({ angleInRadians, width, height }) {
    ...
    const fromX = fixFloat(
      (gradientHeight / 2) * Math.sin(angleInRadians + angle180degInRadians)
    );
    const fromY = fixFloat(
      (gradientHeight / 2) * Math.cos(angleInRadians + angle180degInRadians)
    );
